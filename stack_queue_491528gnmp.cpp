...
int main() {
    ds_491528gnmp::Stack<int> stk;
    for(int x:{ 86, 3, 17, 71, 99, 86, 16, 16, 9, 51 }) stk.push(x);
    std::cout<<"Stack top: "<<stk.peek()<<" size: "<<stk.size()<<"\n";
    ds_491528gnmp::CQueue<int> q(20);
    for(int x:{ 86, 3, 17, 71, 99, 86, 16, 16, 9, 51 }) q.enqueue(x);
