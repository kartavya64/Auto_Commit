...
int main() {
    ds_515941ifmd::Stack<int> stk;
    for(int x:{ 51, 49, 80, 74, 92, 59 }) stk.push(x);
    std::cout<<"Stack top: "<<stk.peek()<<" size: "<<stk.size()<<"\n";
    ds_515941ifmd::CQueue<int> q(20);
    for(int x:{ 51, 49, 80, 74, 92, 59 }) q.enqueue(x);
