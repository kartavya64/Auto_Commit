...
int main() {
    ds_500216bfqu::Stack<int> stk;
    for(int x:{ 67, 11, 88, 85, 56, 77, 2, 3 }) stk.push(x);
    std::cout<<"Stack top: "<<stk.peek()<<" size: "<<stk.size()<<"\n";
    ds_500216bfqu::CQueue<int> q(20);
    for(int x:{ 67, 11, 88, 85, 56, 77, 2, 3 }) q.enqueue(x);
