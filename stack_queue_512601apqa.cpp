...
int main() {
    ds_512601apqa::Stack<int> stk;
    for(int x:{ 24, 36, 75, 93, 99, 41, 6, 35, 23, 65 }) stk.push(x);
    std::cout<<"Stack top: "<<stk.peek()<<" size: "<<stk.size()<<"\n";
    ds_512601apqa::CQueue<int> q(20);
    for(int x:{ 24, 36, 75, 93, 99, 41, 6, 35, 23, 65 }) q.enqueue(x);
